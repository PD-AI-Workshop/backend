...
class TestSuccessCreateDeleteArticleScenario:
    ...
    async def test_scenario(
        ...
        assert_get_all_response(get_all_categories_response, [CategorySchema(**create_category_response.data.model_dump())])

        article_data_to_create = CreateArticleRequestSchema(
            main_image_url=test_file.url,
            image_ids=[test_file.id],
            category_ids=[create_category_response.data.id]
        )
